...
class MeroLaganiAPI():
    ...
    def _parse_table_scrip_details(self, content):
        ...
        for body in table_bodies:
            if len(body.find_all('td')) != 0:
                ...
                value = body.find_all('td')[0].text.replace('\r\n','').replace(' ','').lstrip().rstrip()
    ...
    def _filter_dict(self, unfiltered_dict):
        ...
        to_filter = {'Shares Outstanding': ('shares_outstanding', lambda string: string.replace(',', '') if string else 0, float), 
                     #'Market Price': ('market_price', lambda string: string.replace(',', ''), float_or_zero),
                     'Sector': ('sector', lambda string: string.lower()),
                     '1 Year Yield': ('one_year_yield', lambda string: string[:-1] if string else 0, float),
                     'EPS': ('eps', lambda string: string[:string.index('(')] if string else 0 , float),
                     #'P/E Ratio': ('p_e_ratio', float_or_zero),
                     'Book Value': ('book_value', lambda string: string.replace(',', '') if string and string.count(',') else  string if string else 0, float),
                     #'PBV':('pbv', float_or_zero),
                     '% Dividend': ('dividend', lambda string:0 if not string else string[:string.index('(')].replace(',', '')  if string[:string.index('(')].count(',') else  string[:string.index('(')] , float),
                     '% Bonus': ('bonus', lambda string: string[:string.index('(')] if string else 0 , float),
                     'Right Share': ('right_share', lambda string: (string[string.index(':')+1: string.index('(')] if string[string.index(':')+1: string.index('(')] else 0 ) if string and string.count('(FY') else 0, float),
    ...
    def get_scrip_details(self, scrip_id):
        scrip_details_url = f'{self._get_full_url(self.end_point_scrip_details)}{scrip_id}'
